...
private[
	...
	"_description",
	"_componentQuantity",
	"_componentItemClassName",
	"_componentItemName",
	"_equippedComponentQuantity",
	"_toolItemClassName",
	"_toolItemName",
	"_equippedToolQuantity",
	"_concreteMixer",
	"_interactionModelGroupClassName",
	"_interactionModelGroupConfig",
...
if(isClass(configFile >> "CfgVehicles" >> _pictureItemClassName)) then {
	_pictureItemConfig = configFile >> "CfgVehicles" >> _pictureItemClassName;
	_recipePicture = getText(_pictureItemConfig >> "picture");
};

if(isClass(configFile >> "CfgWeapons" >> _pictureItemClassName)) then {
	_pictureItemConfig = configFile >> "CfgWeapons" >> _pictureItemClassName;
	_recipePicture = getText(_pictureItemConfig >> "picture");
};

if(isClass(configFile >> "CfgAmmo" >> _pictureItemClassName)) then {
	_pictureItemConfig = configFile >> "CfgAmmo" >> _pictureItemClassName;
	_recipePicture = getText(_pictureItemConfig >> "picture");
};
...
_description = format["<t size='1.5' font='PuristaMedium' align='left'>%1</t><br/><br/>", _recipeName];
{
	...
	if(isClass(configFile >> "CfgVehicles" >> _componentItemClassName)) then {
		_componentItemName = getText(configFile >> "CfgVehicles" >> _componentItemClassName >> "displayName");
	};

	if(isClass(configFile >> "CfgWeapons" >> _componentItemClassName)) then {
		_componentItemName = getText(configFile >> "CfgWeapons" >> _componentItemClassName >> "displayName");
	};

	if(isClass(configFile >> "CfgAmmo" >> _componentItemClassName)) then {
		_componentItemName = getText(configFile >> "CfgAmmo" >> _componentItemClassName >> "displayName");
	};
...

{
	...
	if(isClass(configFile >> "CfgVehicles" >> _toolItemClassName)) then {
		_toolItemName = getText(configFile >> "CfgVehicles" >> _toolItemClassName >> "displayName");
	};
	...
	if(isClass(configFile >> "CfgAmmo" >> _toolItemClassName)) then {
		_toolItemName = getText(configFile >> "CfgAmmo" >> _toolItemClassName >> "displayName");
	};
...
{
	_description = _description + format["<t size='1' font='puristaMedium' align='left'>%1</t>", "Base Level"];
