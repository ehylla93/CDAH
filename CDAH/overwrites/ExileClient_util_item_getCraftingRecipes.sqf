...
_availableRecipes = (missionConfigFile >> "CfgCraftingRecipes") call Bis_fnc_getCfgSubClasses;
{
	...
	if(isClass(configFile >> "CfgVehicles" >> _pictureItem)) then {
		_picture = getText(configFile >> "CfgVehicles" >> _pictureItem >> "picture");
	};

	if(isClass(configFile >> "CfgWeapons" >> _pictureItem)) then {
		_picture = getText(configFile >> "CfgWeapons" >> _pictureItem >> "picture");
	};

	if(isClass(configFile >> "CfgAmmo" >> _pictureItem)) then {
		_picture = getText(configFile >> "CfgAmmo" >> _pictureItem >> "picture");
	};

	if(isClass(configFile >> "CfgMagazines" >> _pictureItem)) then {
