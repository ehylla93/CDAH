...
if (_quantityCrafted > -1) then 
{
	...
	{	
		_feedbackMessage = "";
		{
			...
			if(isClass(configFile >> "CfgVehicles" >> _returnedItemClassName)) then {
				_returnedItemName = getText(configFile >> "CfgVehicles" >> _returnedItemClassName >> "displayName");
			};
			...
			if(isClass(configFile >> "CfgAmmo" >> _returnedItemClassName)) then {
				_returnedItemName = getText(configFile >> "CfgAmmo" >> _returnedItemClassName >> "displayName");
			};
